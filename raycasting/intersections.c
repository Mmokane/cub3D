...
/*   Updated: 2023/11/03 17:31:44 by oubelhaj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    get_ray_data(t_data *data, t_ray *ray)
{
	...
	ray->dist_projection_plane = (data->win_width / 2)
		* tan(data->player->fov / 2);
	ray->wall_strip_h = (TILE_SIZE / ray->distance
		* ray->dist_projection_plane);
	ray->wall_cords->top_pix = (data->win_height / 2) - (ray->wall_strip_h / 2);
	if (ray->wall_cords->top_pix < 0)
		ray->wall_cords->top_pix = 0;
	ray->wall_cords->bot_pix = ray->wall_cords->top_pix + ray->wall_strip_h;
	if (ray->wall_cords->bot_pix > data->win_height)
		 ray->wall_cords->bot_pix = data->win_height;
}
...
void	horizontal_intersections(t_ray *ray, t_data *data, t_cords *horz)
{
	...
	if (ray->is_up)
	{
		...
		horz->x_intercept = (data->player->y - horz->y_intercept) * arc_tan +data->player->x;
		horz->y_step = -TILE_SIZE;
	}
	if (ray->is_down)
	{
		horz->y_intercept = (int)data->player->y / TILE_SIZE * TILE_SIZE + TILE_SIZE; 
		horz->x_intercept = (data->player->y - horz->y_intercept) * arc_tan + data->player->x;
		horz->y_step = TILE_SIZE;
	}
	horz->x_step = -horz->y_step * arc_tan;
	while (horz->x_intercept >= 0 &&  horz->x_intercept <= data->win_width * TILE_SIZE && horz->y_intercept >= 0 && horz->y_intercept <= data->win_height * TILE_SIZE)
	{
...
void	vertical_intersections(t_ray *ray, t_data *data, t_cords *vert)
{
	...
	if (ray->is_left)
	{
		...
		vert->y_intercept = (data->player->x - vert->x_intercept) * arc_tan +data->player->y;
		vert->x_step = -TILE_SIZE;
	}
	if (ray->is_right)
	{
		vert->x_intercept = (int)data->player->x / TILE_SIZE * TILE_SIZE + TILE_SIZE; 
		vert->y_intercept = (data->player->x - vert->x_intercept) * arc_tan + data->player->y;
		vert->x_step = TILE_SIZE;
	}
	vert->y_step = -vert->x_step * arc_tan;
	while (vert->x_intercept >= 0 &&  vert->x_intercept <= data->win_width * TILE_SIZE && vert->y_intercept >= 0 && vert->y_intercept <= data->win_height * TILE_SIZE)
	{
		if (is_wall(vert->x_intercept - (ray->is_left), vert->y_intercept, data))
		{
			vert->hit = 1;
			return ;
		}
